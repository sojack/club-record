...
- [ ] **Step 3: `lib/csv-parser.ts` — option unions.** In `RelayParseOptions` and `RelayTemplateOptions`, change `scope?: "club" | "national_provincial";` to:
```ts
  scope?: "club" | "provincial" | "national" | "national_provincial";
```
(both interfaces — there are two `scope?:` lines, ~L80 and ~L274).
...
- [ ] **Step 2: bulk-upload — derive scope from club level.** Add `import { scopeForClubLevel } from "@/lib/scope";`. Where the file maps a parsed file to the `parseRecordsCSV` call and the `record_lists` insert: compute once `const listScope = scopeForClubLevel(selectedClub?.level);`. Pass `scope: listScope` into the `parseRecordsCSV(content, { relay: …, scope: listScope })` options, and set the `record_lists` insert's `scope: listScope` (replacing `scope: file.scope`). Remove `scope` from the `parsed.push({...})` object literal and the `const { …, scope } = parseFilename(...)` destructure.
...
- [ ] **Step 5: CSVUploader — scope union + provincial template/columns.** In `components/CSVUploader.tsx`: widen the `scope?:` prop union to `"club" | "provincial" | "national" | "national_provincial"`. Where it builds the expected-columns hint / template options based on `scope === "national_provincial"`, change the predicate so **Province is included only for `national`** and Club/AgeGroup for `provincial` + `national`: replace `scope === "national_provincial"` checks with `const wantsProvince = scope === "national" || scope === "national_provincial"; const wantsClub = scope !== "club";` and use `wantsClub`/`wantsProvince` to choose the relay/individual header & hint (provincial → AgeGroup+Club, no Province; national → AgeGroup+Club+Province; club → none). Keep behaviour identical for `national`/legacy.
